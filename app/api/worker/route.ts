...
export async function POST(request: Request) {
  const token = request.headers.get("authorization") ?? "",
    expected = `Bearer ${process.env.WORKER_SECRET ?? ""}`;
  if (
  ...
  const db = getAdminSupabase(),
    base = process.env.MESSAGING_PROVIDER_BASE_URL!,
    key = process.env.MESSAGING_PROVIDER_API_KEY;
  if (!key)
  ...
  const d = jobs?.[0];
  if (!d) return Response.json({ idle: true });
  try {
    ...
    if (!batchId) {
      ...
      if (!response.ok)
        throw new Error(
          `Dispatch HTTP ${response.status}; same approval will retry`,
        );
      ...
      if (
        ...
        throw new Error("Invalid dispatch response; same key will retry");
    ...
    for (let page = 0; page < 20 && Date.now() - started < 20000; page++) {
      const response = await fetch(
        `${base}/v1/messages/${encodeURIComponent(batchId)}/events${cursor ? `?since=${encodeURIComponent(cursor)}` : ""}`,
        {
      ...
      const body = await response.json();
      if (!Array.isArray(body.events)) throw new Error("Invalid event page");
      const audience = new Set(
        d.recipients.map((r: { external_id: string }) => r.external_id),
      );
      const rows = [];
      for (const raw of body.events) {
        try {
          const event = normalizeEvent(raw);
          const code = String(raw.brand_code ?? "").toLowerCase();
          if (
            !audience.has(event.contact_id) ||
            (["kilele", "karoo", "marrakech"].includes(code) &&
              code !== brand?.slug)
          )
            throw new Error("Report outside approved brand/audience");
          rows.push({ ...event, dispatch_id: d.id, brand_id: d.brand_id });
        } catch (error) {
          warnings[String(raw.event_id ?? raw.id ?? "malformed")] =
      ...
      if (rows.length) {
        const { error: e } = await db
          ...
          .upsert(rows, {
            onConflict: "dispatch_id,event_id",
      ...
      const waiting =
        ...
        (body.next_cursor == null ||
          String(body.next_cursor) === String(cursor));
      if (body.has_more) {
        if (!waiting) cursor = String(body.next_cursor);
    ...
    await db
      ...
      .update({
        ...
        next_attempt_at: new Date(
          Date.now() + Math.min(900000, 30000 * 2 ** Math.min(d.attempts, 5)),
        ).toISOString(),
      })
      .eq("id", d.id);
