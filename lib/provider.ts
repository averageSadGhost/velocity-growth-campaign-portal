...
export function normalizeEvent(raw: Record<string, unknown>): Report {
  ...
  const recipient = raw.recipient as
    | Record<string, unknown>
    | string
    | undefined;
  const contact_id = String(
    raw.external_contact_id ??
      raw.contact_id ??
      raw.recipient_id ??
      (typeof recipient === "object"
        ? (recipient.external_id ?? recipient.id)
        : recipient) ??
      "",
  );
  const value = String(
    raw.event_type ?? raw.type ?? raw.event ?? "",
  ).toLowerCase();
  const kind =
    (
      {
  ...
  const occurred_at = String(
    raw.occurred_at_utc ?? raw.occurred_at ?? raw.timestamp ?? "",
  );
  if (
    !event_id ||
    !contact_id ||
    ...
    throw new Error("Unrecognized provider event; cursor not advanced");
